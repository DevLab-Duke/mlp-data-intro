...
plot_variable = function(country, event){
  ...
  choices_df = data.frame(
    id = c("arrestNorm", "censorNorm", "corruptionNorm", "activismNorm", "cooperateNorm", "defamationcaseNorm", "legalactionNorm", "legalchangeNorm", "protestNorm", "purgeNorm", "raidNorm", "threatenNorm", "violencelethalNorm", "violencenonlethalNorm", "mobilizesecurityNorm"),
    names = c("Arrests", "Censorship", "Corruption", "Civic Activism", "Political Cooperation", "Defamation Cases", "Legal Actions","Legal Changes",  "Protests", "Purges", "Raids", "Political Threats","Lethal Violence", "Non-lethal Violence", "Mobilize Troops") 
  )
  
  # Read-in data
  # categories = dat
  categories = readr::read_csv(here::here("data", "final-counts", paste0(country,".csv")))
