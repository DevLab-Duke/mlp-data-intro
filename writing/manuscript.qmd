...
thanks: This study was funded by the United States Agency for International Development (USAID) Bureau for Democracy, Human Rights, and Governance and the Open Society Foundations. Check out our data dashboards at [mlpeace.org](https://web.sas.upenn.edu/mlp-devlab/civic-space-data-and-forecasts/).
...
In 2016, approximately 3.5 billion people lived under autocracy; by 2021, this number had surged to over 5.4 billion [@boese2022autocratization]. Concentrated in the global south, this "third wave of autocratization" is constricting civic space^[Following @brechenmacher2019civic, we define civic space as the fundamental freedoms that allow people to gather, communicate, and take part in groups to influence society and politics.] and limiting the ability of citizens to advocate for better governance [@luhrmann2019third; @waldner2018unwelcome]. Nevertheless, citizens worldwide continue to challenge these authoritarian movements.

Despite the importance of this day-to-day push-and-pull over citizen liberties and state control, data to study the events and processes that constitute this struggle is limited. Existing measures of civic space come largely from annual, expert-coded indicators classifying the nature of political regimes. While these regime indices have opened-up new domains of research to rigorous investigation, they are not designed to provide insight into the quotidian politics where battles over civic space take place.

This article introduces the Machine Learning for Peace (ML4P) dataset, which provides **monthly** data on 20 civic space events across 62 developing countries from 2012 through 2024. *ML4P* measures civic space activity by capturing monthly variation in levels of media attention across events, providing a dynamic view of where and when civic space is changing. *ML4P* represents a major advance in our ability to understand civic space dynamics by providing a higher-frequency measure of a broad range of events bearing on civic space. Using a carefully assembled corpus of online news, *ML4P* also captures the significance of these events to the domestic political environment.

Importantly, *ML4P* is constructed from articles collected by the High-Quality Media from Aid Receiving Countries (*HQMARC*) corpus, an original repository of online news from 16 international and 12 regional sources, as well as a curated sample of nearly 350 prominent domestic media outlets based across our sample of countries. Rather than using web-crawlers or pre-canned scraping tools, *HQMARC* uses human-supervised custom web scraping and parsing to carefully capture each outlet's complete publication history. This human-supervised approach is particularly important for domestic news sources, whose websites are often less stable than international ones but which provide far more coverage of domestic politics.

*HQMARC* has major advantages over big data media aggregators relying on web-crawlers and general scraping tools, such as GDELT, Common Crawl, and Internet Archive. By focusing on a curated list of high-quality sources, *HQMARC* achieves a stable, well-understood composition that allows researchers to measure changes in the proportion of media attention devoted to 20 types of events within countries over time. *HQMARC* also surpasses expensive commercial options, such as LexisNexis, in terms of linguistic diversity and coverage of many developing countries.

*HQMARC* includes more than 150 million articles published in more than 40 languages, making human classification prohibitively expensive. To produce *ML4P's* structured data on 20 civic space events from this large volume of multi-lingual text, we use open-source computational tools to translate and extract information from each article, identifying the main event being reported on and the country in which event occurs.
...
Ultimately, what appear as slow-moving changes in regimes are the result of government and citizen actions that happen at specific moments in time. Existing measures provide aggregations of those changes over 12-month periods. Our project complements those efforts by measuring the events -- often occurring over days or weeks -- that contribute to broad, prolonged processes of democratic backsliding. We specifically seek to cover civic events that might be lost in the process of aggregating indices and sub-indices annually. Take, for instance, the widespread move across U.S. states in recent years to restrict ballot access, reduce the number of polling places, and increase voter registration requirements. Each of these involves a host of important events: legal changes, administrative decisions, state supreme court decisions, etc. Even if each of those individual events are meaningful for the evolution of democracy in the US, they are averaged across other state and federal events over the year to induce only a modest change in annual regime indices.

ML4P increases the granularity of data -- both temporally and substantively -- that is relevant to broader debates on regime dynamics and backsliding. In doing so, we shift the focus to 'civic space', which we conceptualize as the day-to-day practices through which citizens organise, participate and communicate with each other over the structures that govern them. As a measurement strategy, this analytical shift involves a movement from summary indices to coverage of civically salient events. There are several large event data projects that are relevant to civic space, such the Armed Conflict Location Event Data Project (ACLED), the Uppsala Conflict Data Project Georeferenced Event Dataset (UCDP GED), and the Political Event Classification, Attributes, and Types (Polecat) dataset.

From the point of view of measuring civic activity, these projects has limitations that we overcome. First, they are focused on violence events (ACLED, UCDP, Polecat) and thus provide a very partial range of the activities impacting civic space. Second, these data either rely on relatively expensive and slow human coding (ACLED, UCDP) or only on international news sources (Polecat) that provide weak coverage of civic space in many countries.

Encompassing the breadth of politically-relevant civic space events is difficult. While we have a rich body of theory about 'regimes', the literature on 'civic space' and 'civil society' is spread across varied bodies of work on protest, social capital, legal studies, and election studies. Our solution is to collect data on a large number of different civic space events. In consultation with academic research and policy practitioners, we define 20 civic space event types. They are: activism, arrests, censorship, cooperation, corruption, coups, defamation cases, election activity, election irregularities, legal actions, legal changes, lethal violence, martial law, the mobilization of security forces, non-lethal violence, protests, bureaucratic purges, raids, and threats. We also code disasters given the propensity of aspiring autocrats to use them (particularly COVID) as justification for restricting civil society. While some of these event types, such as protests and coups, are subject to systematic data collection elsewhere, our approach produces the first systematic data on many of them that we know of. Together, these event types provide a rich picture of the monthly contest over civic space and offer the potential for new research on civic dynamics.
