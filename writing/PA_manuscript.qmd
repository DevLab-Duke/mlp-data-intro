...
Appendix A provides comprehensive documentation of the *HQMARC* corpus. It provides the geographic distribution of domestic and regional media outlets across our sample, the linguistic diversity of the corpus, and the inventory of news sources by country. It also includes a case study validating our harvesting workflow by comparing *HQMARC* to large "big-data" corpora -- specifically GDELT and Internet Archive -- showing that HQMARC captures more articles from leading domestic outlets while avoiding the parsing and metadata errors common to generalized crawlers.

<!-- diego moved this to the appendix (Appendix A section 5)
...
[^5]: For Tetum, no suitable model existed initially, but an open-source one emerged after several months.
...
To classify articles into 20 civic space event categories, we fine-tuned an open-source, transformer-based RoBERTa large language model [@liu2019roberta].\footnote{Recent research shows that closed-source LLMs perform only moderately better, at far higher cost [@deandrade2024], while RoBERTa performs well for most political science applications [@timoneda2024roberta].} To fine-tune the model, we built a double human-coded training dataset of 6,475 newspaper articles originally published in both English and non-English languages. As reported in Appendix B, the model achieves 82% out-of-sample accuracy -- comparable to intercoder reliability -- with most misses coming from articles reporting on multiple events. To reduce noise associated with background and contextual content, we classify only the title and first 600 characters.\footnote{This corresponds to the title and first 2–3 sentences. Extensive testing suggests that adding more text reduces performance by introducing context that obscures the main event.} We also allow dual classifications for overlapping events (e.g., corruption with arrests). For several categories, we further apply a targeted keyword filter to eliminate common false-positives. See Appendix B Section 1 for a definition and examples for each event category and Appendix B Section 2 for a description of keyword filtering.

We subsequently deploy a second classifier to exclude articles that fit an event category but lack political relevance. For example, our first model cannot distinguish politically salient arrests from those involving apolitical criminal activity. To address this, we fine-tuned a 'political relevance' classifier using transfer learning from our RoBERTa model and a double human-coded dataset of 2,938 articles. This model achieves 0.87 accuracy. This model assigns a binary output (0/1) indicating each article’s political relevance.
...
An example from Indonesia illustrates the point. Between March and November 2024, the country saw major corruption scandals -- including the PT Timah state enterprise corruption case in March, the Tom Lembong sugar import and Sahbirin Noor cases in October, and the Rohidin Mersyah case in November. In May alone, the *HQMARC* domestic outlets published over 970 articles on corruption, while regional and international sources carried *none*. This discrepancy shows how relying on international media can obscure the salience of civic events and even miss them entirely. Across our dataset, we find 59 such shocks in 23 countries with zero international coverage.

## Major Event Validation

To validate our data’s ability to detect major political events, we adopt two approaches. First, we assess whether months flagged as shocks align with real-world developments (i.e., true positives). We randomly selected five countries -- Kosovo, Morocco, Angola, Mauritania, and Ukraine -- spanning three regions and four languages (Serbian, Albanian, French, and Ukrainian). Across the three most recent months (300 possible event-months), our model flagged 40 as exhibiting a shock. Because such months often include multiple events, we reviewed the top five per event-month. In 38 of 40 cases (95%), all top-five events matched the correct category (see Appendix F). 

Our second approach identifies major world events expected to generate shocks in the *ML4P* data and tests whether they appear in the data (avoiding false negatives). We do so by: (1) examining a single historical event likely to generate media attention on one of our *ML4P* event categories across countries, and (2) assessing whether both frequent and rare political events produce detectable spikes in relevant event categories.
...
![Elections and Electoral Activity (Latin America and the Caribbean). The vertical lines mark key milestones in each country’s electoral cycle, such as primaries, congressional, or presidential elections.](event_validation/Combined_elections_LAC.png){#fig-elections_lac}
...
As democratic institutions confront unprecedented challenges in an era of autocratization, understanding the day-to-day dynamics of civic space has become critical for both research and policy. This paper addresses a key gap by introducing *ML4P* -- the first comprehensive, high-frequency measure of civic space events across developing countries.
...
Validation exercises confirm that *ML4P* captures real-world political dynamics, from COVID-19 lockdowns and electoral cycles to coups and democratic crises. We also document systematic biases in international media: correlations with domestic reporting are weak, and major events are often entirely absent. These findings underscore the limitations of datasets reliant on international sources. The predictive utility of *ML4P*  is demonstrated through our travel advisory forecasting model, which shows the data capture early warning signals of instability rather than mere reporting noise.

*ML4P* opens important avenues for future research. First, *ML4P* enables an analytical and empirical shift in the study of democratic backsliding by providing the temporal granularity necessary to trace how fast-paced events contribute to broader regime changes. It allows researchers to trace not just *when* regime change occurs but *how* (e.g., which elite strategies are deployed in sequence, and under what conditions). Second, the dataset also supports research on contentious politics, media behavior in backsliding regimes, corruption, and political violence. And third, it allows testing of variation across cases: which democracies resist backsliding, how polarization spikes around key civic events, and how international shocks translate into domestic political behavior. 

For policymakers, our findings highlight the critical importance of incorporating domestic media into monitoring and assessment. Reliance on international sources risks fundamental misreadings of political conditions. Investment in human-supervised infrastructures, as demonstrated by *HQMARC*, provides an essential complement to automated systems.

Future research could expand *ML4P's* geographic coverage and link it to other high-frequency indicators to enhance our understanding of how civic space dynamics intersect with economic conditions, social movements, and international interventions. More than a dataset, *ML4P* offers a new lens for understanding how democracy erodes and civic space contracts in real time. As authoritarian movements challenge democratic institutions worldwide, tools like *ML4P* become essential for documenting these processes and informing effective responses to defend civic space and democratic governance.
