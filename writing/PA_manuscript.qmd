...
This paper proceeds in six parts. Section 2 discusses how *ML4P* complements existing regime data and opens new research avenues. Section 3 describes our data production process and approach advantages, comparing *HQMARC's* superior coverage to other major media repositories.  In the following validation section we briefly describe the data before we comparing event coverage from international sources to that of domestic sources. We find systematic differences in the types of events they cover and document significant events that international sources ignore. Our findings have implications for event data that is generated from predominantly international media outlets, a common practice in the social sciences. we also report on a series of validation exercises that confirm *ML4P's* quality. Thereafter, we provide a use case, documenting that *ML4P's* civic space events are predictive of monthly US State Department (DOS) travel advisory onsets. DOS travel advisories capture a broad range of security, health, and civic threats; as rare events, travel advisory onsets are a difficult target variable. The final sections discuss *ML4P's* limitations and why, despite them, it represents a valuable resource for research on democratic backsliding, contentious politics, media behavior, and crisis response.
...
[^2]:Adding or dropping sources introduces the possibility that reporting volume trends for specific events are artifacts of source material changes rather than true event salience changes. Similarly, ACLED's documentation notes that "... the addition of such a source in an ad hoc fashion risks the integrity of historical trends as it will introduce an 'artificial spike' in the data. This refers to the phenomenon where if that same source was first back-coded before being introduced into the data, the 'spike' that its inclusion introduces in the data would be gone (or minimized) — suggesting that the spike does not reflect a 'true spike' in disorder on the ground"[@acled_adding_sources_2023].
...
The result is a flexible research infrastructure that balances coverage breadth, source quality, and processing scalability. @fig-mermaid provides a graphic representation of the *ML4P* data production pipeline. In the remainder of this section, we describe each step in said pipeline.
...
*ML4P* is constructed by processing articles from the *HQMARC* corpus. A key advantage of *HQMARC* is its unprecedented accuracy and granularity in capturing the publication history of critical domestic media outlets. To overcome the composition challenges discussed above, we developed an infrastructure designed to comprehensively capture sources' full publication history and maintain accurate metadata. This process involves three main steps:

1.  **Identify High-Quality Domestic Sources:** We compile a list of local news sources with machine-scrapable websites by consulting directories of each country's media market (e.g., university library guides, Reporters Without Borders) as well as partners working in international NGOs and local civil society organizations. We conduct a desk review of each source's partisan affiliation by consulting reports on media ownership in the outlet's country (see Appendix A Section 4). In very repressive contexts, we occasionally include sources based outside their home countries. For example, we include *El Faro*, a leading Salvadoran independent outlet that relocated to Costa Rica due to government persecution.
...
Appendix A provides comprehensive documentation of the *HQMARC* corpus, including geographic distribution of domestic and regional media outlets, linguistic diversity, and news source inventory by country. It also includes a case study validating our harvesting workflow by comparing *HQMARC* to large "big-data" corpora -- specifically GDELT and Internet Archive -- showing that *HQMARC* captures more articles from leading domestic outlets while avoiding the parsing and metadata errors common to generalized crawlers.
...
Both international and domestic outlets report on events taking place across many countries. We identify all locations mentioned in our recovered text. If no country is found, we assign the article to the country in which the publishing outlet is based. For international and regional outlets, articles are only assigned to a country if they explicitly mention a location within that country in the first 600 characters. For domestic sources, we use the CLIFF-CLAVIN geoparser with the GeoNames ontological gazetteer to identify geographic entities (e.g., states, cities) mentioned in the text.[^6] For each location, we use the CLIFF API to retrieve the location's country code and assign the article to that country.
...
We subsequently deploy a second classifier to exclude articles that fit an event category but lack political relevance. For example, our first model cannot distinguish politically salient arrests from apolitical criminal activity. To address this, we fine-tuned a 'political relevance' classifier using transfer learning from our RoBERTa model and a double human-coded dataset of 2,938 articles. This model achieves 0.87 accuracy and assigns binary output (0/1) indicating each article's political relevance.
...
In this section, we demonstrate *ML4P's* ability to capture civic space dynamics in an important forecasting application. That high-frequency civic space indicators help predict the issuance of independent security assessments -- U.S. Department of State (DOS) high-level travel advisories (HLTAs) -- provides further validation that MLP's data captures meaningful underlying political conditions. 
...
Second, domestic sources more difficult to scrape are less likely included. Some sources in low-resource countries have extremely poor website architecture, making scraping extremely difficult. Third, news organizations have biases -- coverage is much stronger in cities than rural areas. Despite these limitations, *HQMARC's* focus on domestic media paints a richer picture than international media coverage provides. 

Two cautions arise from *ML4P's* reliance on media attention as event importance proxy. First, media coverage is influenced by editorial priorities, political pressures, and audience interests rather than objective event significance. Critical events may receive limited coverage due to competing news cycles, censorship, or media ownership structures. Second, our normalization approach (dividing event counts by total coverage) forces competition between event categories within any month. As coverage of some event types increases, others necessarily decrease. When exceptionally large events dominate media coverage other significant but less dramatic events appear relatively less important in our data.

# 7. Conclusion {#sec-conclusion}

The global trend of democratic erosion underscores the need for data that captures the everyday struggles shaping civic space. Existing annual regime indices remain indispensable, but they inevitably smooth over the fast-moving political events that drive longer-term institutional change. The *ML4P* dataset addresses this gap by combining a curated corpus of domestic media with state-of-the-art natural language processing, yielding monthly measures of 20 distinct civic space event types across the developing world.

Our contribution is both empirical and methodological. Empirically, *ML4P* provides unprecedented coverage: monthly data constructed from over 120 million articles across `r sum_lengths` hand-picked domestic outlets in `r nl` languages, with 95% sourced domestically rather than internationally. Methodologically, we demonstrate that human-supervised web scraping combined with transformer models achieves comprehensive coverage and high accuracy while remaining cost-effective.

Validation exercises confirm that *ML4P* captures real-world dynamics from COVID-19 lockdowns to coups and democratic crises. We also document systematic biases in international media: correlations with domestic reporting are weak, and major events are often entirely absent. These findings underscore the limitations of datasets reliant on international sources. The predictive utility of *ML4P*  is demonstrated through our travel advisory forecasting model, which shows the data capture early warning signals of instability rather than mere reporting noise.

*ML4P* enables three critical research advances. First, it provides the temporal granularity necessary to trace how fast-paced events contribute to regime change allowing researchers to trace not just *when* regime change occurs but *how*. Second, the dataset also supports research on contentious politics, media behavior in backsliding regimes, corruption, and political violence. And third, it allows testing of variation across cases: which democracies resist backsliding, how polarization spikes around key civic events, and how international shocks translate into domestic political behavior. 
...
Future research could expand *ML4P's* geographic coverage and link it to other high-frequency indicators, enhancing understanding of how civic space dynamics intersect with economic conditions, social movements, and international interventions. More than a dataset, ML4P offers a new lens for understanding how democracy erodes in real time. As authoritarian movements challenge democratic institutions worldwide, tools like ML4P become essential for documenting these processes and informing effective responses to defend civic space and democratic governance.
